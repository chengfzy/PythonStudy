...
class ProgressBoard(d2l.HyperParameters):
    ...
    def draw(self, x, y, label, every_n=1):
        ...
        for (k, v), ls, color in zip(self.data.items(), self.ls, self.colors):
            plt_lines.append(d2l.plt.plot([p.x for p in v], [p.y for p in v], linestyle=ls, color=color)[0])
            labels.append(k)
...
class TimeMachine(d2l.DataModule):
    ...
    def __init__(self, batch_size, num_steps, num_train=10000, num_val=5000):
        ...
        array = d2l.tensor([corpus[i : i + num_steps + 1] for i in range(len(corpus) - num_steps)])
    ...
    def __init__(self, vocab_size, num_hiddens, ffn_num_hiddens, num_heads, num_blks, dropout, use_bias=False):
        ...
        for i in range(num_blks):
            self.blks.add_module(
                "block" + str(i), TransformerEncoderBlock(num_hiddens, ffn_num_hiddens, num_heads, dropout, use_bias)
            )
...
def show_trace_2d(f, results):
    ...
    x1, x2 = d2l.meshgrid(d2l.arange(-5.5, 1.0, 0.1), d2l.arange(-3.0, 1.0, 0.1), indexing='ij')
...
def multibox_prior(data, sizes, ratios):
    ...
    w = (
        torch.cat((size_tensor * torch.sqrt(ratio_tensor[0]), sizes[0] * torch.sqrt(ratio_tensor[1:])))
        * in_height
        / in_width
...
def reorg_test(data_dir):
    ...
    for test_file in os.listdir(os.path.join(data_dir, 'test')):
        copyfile(
            os.path.join(data_dir, 'test', test_file), os.path.join(data_dir, 'train_valid_test', 'test', 'unknown')
        )
...
class BERTEncoder(nn.Module):
    ...
    def __init__(self, vocab_size, num_hiddens, ffn_num_hiddens, num_heads, num_blks, dropout, max_len=1000, **kwargs):
        ...
        for i in range(num_blks):
            self.blks.add_module(
                f"{i}", d2l.TransformerEncoderBlock(num_hiddens, ffn_num_hiddens, num_heads, dropout, True)
            )
...
class BERTModel(nn.Module):
    ...
    def __init__(self, vocab_size, num_hiddens, ffn_num_hiddens, num_heads, num_blks, dropout, max_len=1000):
        ...
        self.encoder = BERTEncoder(
            vocab_size, num_hiddens, ffn_num_hiddens, num_heads, num_blks, dropout, max_len=max_len
        )
    ...
    def update(self, config: dict, error: float, info=None):
        ...
        if ri < self.r_max:
            ...
            if len(self.observed_error_at_rungs[ri]) >= ni:
                ...
                best_performing_configurations = self.get_top_n_configurations(rung_level=ri, n=niplus1)
                riplus1 = self.rung_levels[self.K - kiplus1]  # r_{i+1}
                # Queue may not be empty: insert new entries at the beginning
                self.queue = [
                    dict(config, max_epochs=riplus1) for config in best_performing_configurations
                ] + self.queue
...
def show_value_function_progress(env_desc, V, pi):
    ...
    for k in range(V.shape[0]):
        ...
        action2dxdy = {0: (-0.25, 0), 1: (0, 0.25), 2: (0.25, 0), 3: (-0.25, 0)}
